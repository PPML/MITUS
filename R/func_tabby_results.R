...
#'       #total population
#' ################################################################################
#'       ResTab[1:nr+(intv-1)*100,,1,ag]<-o[,age_id,1]
#'       #number of ltbi prevalence
#'       ResTab[1:nr+(intv-1)*100,,2,ag]<-apply(o[,age_id,c(54,65)+ag],c(1,2),sum)*1e3
#'       #percentage of ltbi prevalence
#'       ResTab[1:nr+(intv-1)*100,,3,ag]<-apply(o[,age_id,c(54,65)+ag],c(1,2),sum)/o[, age_id,2+ag]*1e2
#'       #TB notifications (alive+dead at diagnosis)
#'       ResTab[1:nr+(intv-1)*100,,4,ag]<-apply(o[,age_id,c(135,188)+ag],c(1,2),sum)*1e3
#'       #percentage TB notifications (alive+dead at diagnosis)
#'       ResTab[1:nr+(intv-1)*100,,5,ag]<-apply(o[,age_id,c(135,188)+ag],c(1,2),sum)/o[, age_id,2+ag]*1e2
#'       #tb attributable deaths
#'       ResTab[1:nr+(intv-1)*100,,6,ag]<-apply(o[,age_id,c(87,98)+ag],c(1,2),sum)*1e3
#'       # percentage tb attributable deaths
#'       ResTab[1:nr+(intv-1)*100,,7,ag]<-apply(o[,age_id,c(87,98)+ag],c(1,2),sum)/o[, age_id,2+ag]*1e2
#'
#'       ################################################################################
#'       #US Born population
#'       ################################################################################
#'       ResTabus[1:nr+(intv-1)*100,,1,ag]<-o[,age_id,1]
#'       #number of ltbi prevalence
#'       ResTabus[1:nr+(intv-1)*100,,2,ag]<-o[, age_id,54+ag]*1e3
#'       #percentage of ltbi prevalence
#'       ResTabus[1:nr+(intv-1)*100,,3,ag]<-o[, age_id,54+ag]/o[, age_id,2+ag]*1e2
#'       #TB notifications (alive+dead at diagnosis)
#'       ResTabus[1:nr+(intv-1)*100,,4,ag]<-apply(o[, age_id,c(204,215)+ag],c(1,2),sum)*1e3
#'       #percentage TB notifications (alive+dead at diagnosis)
#'       ResTabus[1:nr+(intv-1)*100,,5,ag]<-apply(o[, age_id,c(204,215)+ag],c(1,2),sum)/o[, age_id,2+ag]*1e2
#'       #tb attributable deaths
#'       ResTabus[1:nr+(intv-1)*100,,6,ag]<-o[, age_id,87+ag]*1e3
#'       # percentage tb attributable deaths
#'       ResTabus[1:nr+(intv-1)*100,,7,ag]<-o[, age_id,87+ag]/o[, age_id,2+ag]*1e2
#'
#'       ################################################################################
#'       #non-US Born population
#'       ################################################################################
#'       ResTabfb[1:nr+(intv-1)*100,,1,ag]<-o[,age_id,1]
#'       #number of ltbi prevalence
#'       ResTabfb[1:nr+(intv-1)*100,,2,ag]<-o[, age_id,65+ag]*1e3
#'       #percentage of ltbi prevalence
#'       ResTabfb[1:nr+(intv-1)*100,,3,ag]<-o[, age_id,65+ag]/o[, age_id,2+ag]*1e2
#'       #TB notifications (alive+dead at diagnosis)
#'       ResTabfb[1:nr+(intv-1)*100,,4,ag]<-(apply(o[, age_id,c(135,188)+ag],c(1,2),sum)-apply(o[, age_id,c(204,215)+ag],c(1,2),sum))*1e3
#'       #percentage TB notifications (alive+dead at diagnosis)
#'       ResTabfb[1:nr+(intv-1)*100,,5,ag]<-(apply(o[, age_id,c(135,188)+ag],c(1,2),sum)-apply(o[, age_id,c(204,215)+ag],c(1,2),sum))/o[, age_id,43+ag]*1e2
#'       #tb attributable deaths
#'       ResTabfb[1:nr+(intv-1)*100,,6,ag]<-o[, age_id,98+ag]*1e3
#'       # percentage tb attributable deaths
#'       ResTabfb[1:nr+(intv-1)*100,,7,ag]<-o[, age_id,98+ag]/o[, age_id,2+ag]*1e2
#'     }
...
#'           }}}}}}
#'
#'   save(res_tab2, file="restab2.rda")
