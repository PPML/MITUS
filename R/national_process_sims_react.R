...
#'@name  national_OutputsZint_react
...
#'@export
national_OutputsZint_react <-  function(samp_i=1,ParMatrix,loc, startyr=1950, endyr=2050,Int1=0,Int2=0,Int3=0,Int4=0,Int5=0,Scen1=0,Scen2=0,Scen3=0,prg_chng, ttt_input) {
  ...
  m <- national_cSim_react( nYrs       = endyr-(startyr-1)         , nRes      = length(func3_ResNam())  , rDxt     = prms[["rDxt"]]  , TxQualt    = prms[["TxQualt"]]   , InitPop  = prms[["InitPop"]]    ,
...
#'@name  national_OutputsInt_react
#'@param loc two-digit abbreviation for location
...
#'@return out outputs
#'@export
national_OutputsInt_react <- function(loc,ParMatrix,n_cores=1,endyr=2050,Int1=0,Int2=0,Int3=0,Int4=0,Int5=0,Scen1=0,Scen2=0,Scen3=0, prg_chng, ttt_input) {
  if(min(dim(as.data.frame(ParMatrix)))==1) {
    out <- national_OutputsZint_react(samp_i=1,ParMatrix=ParMatrix,loc=loc,endyr=endyr,Int1=Int1,Int2=Int2,Int3=Int3,Int4=Int4,Int5=Int5,Scen1=Scen1,Scen2=Scen2,Scen3=Scen3, prg_chng=prg_chng, ttt_input=ttt_input)
  } else {
    out0 <- mclapply(X=1:nrow(ParMatrix),FUN=national_OutputsZint_react,mc.cores=n_cores,
