...
notif_fb_lik <- function(V) {
  ...
  adj_1         <- sum(dnorm(notif_fb,notif_fb,notif_fb*0.1/1.96,log=T)*wts[44:70])
...
notif_us_lik <- function(V) {
  ...
  adj_1         <- sum(dnorm(notif_us,notif_us,notif_us*0.1/1.96,log=T)*wts[44:70])
...
#'@return likelihood
notif_fb_lLik <- function(V,rho=0.1) {
  notif_fb      <- cbind(CalibDat[["age_cases_fb"]][,12],CalibDat[["age_cases_us"]][,12])/1e6
  adj_3         <- sum(dDirMult(M=notif_fb,n=notif_fb,Rho=rho)*wts[44:70])
  (sum(dDirMult(M=V,n=notif_fb,Rho=rho)*wts[44:70]) - adj_3 )*2 }
...
#'@param rho correlation parameter
#'@return likelihood
notif_age_us_lLik <- function(V,rho=0.1) {
...
#'@param rho correlation parameter
#'@return likelihood
notif_age_fb_lLik <- function(V,rho=0.1) {
...
#'@param V table of notifications by fb 2015-2019 (row=22 years, col=fb then us)
#'@return likelihood
notif_fbus_slp_lLik <- function(V) {
  notif_age_fb0     <- CalibDat[["age_cases_fb"]][23:27,12]
  notif_age_us0     <- CalibDat[["age_cases_us"]][23:27,12]
...
#'@param rho correlation parameter
#'@return likelihood
notif_us_hr_lLik <- function(V,rho=0.1) {
...
notif_hr2_lLik <- function(V,rho=0.1) {
  ...
  adj_5d<-sum(dnorm(notif_hr2,notif_hr2,notif_hr2*0.1/1.96,log=T)*wts[44:70])
  (sum(dnorm(notif_hr2,V*1e6,notif_hr2*0.1/1.96,log=T)*wts[44:70]) - adj_5d)
}
...
#'@param rho
#'@return likelihood
notif_hr_dist_lLik<-function(V,rho=0.1){
  ...
  adj_5c <-sum(dDirMult(M=notif_hr_dist,n=notif_hr_dist,Rho=rho)*wts[44:70])
...
ltbi_us_11_lLik <- function(V) {
  ...
  V[9,] <- colSums(V[9:11,])
  (sum( dbeta(V[2:9,1]/rowSums(V[2:9,]),ltbi_us_11[,2],ltbi_us_11[,3],log=T) ) - adj_15)  }
...
ltbi_us_11_dp_lLik <- function(V) {
  ...
  V[9,] <- colSums(V[9:11,])
  (sum( dbeta(V[2:9,1]/rowSums(V[2:9,]),ltbi_us_11_dp[,2],ltbi_us_11_dp[,3],log=T) ) - adj_15dp)  }
...
ltbi_fb_11_lLik <- function(V) {
  ...
  V[9,] <- colSums(V[9:11,])
  (sum( dbeta(V[2:9,1]/rowSums(V[2:9,]),ltbi_fb_11[,2],ltbi_fb_11[,3],log=T) ) - adj_16)  }
...
ltbi_fb_11_dp_lLik <- function(V) {
  ...
  V[9,] <- colSums(V[9:11,])
  (sum( dbeta(V[2:9,1]/rowSums(V[2:9,]),ltbi_fb_11_dp[,2],ltbi_fb_11_dp[,3],log=T) ) - adj_16dp)  }
...
tb_dth_tot_lLik <- function(V) {
  ...
  adj_19a         <- sum(dnorm(tb_deaths_tot,tb_deaths_tot,tb_deaths_tot*0.2/1.96,log=T)*wts[50:69])
  V2<-rowSums(V)
  sum(dnorm(tb_deaths_tot,V2*1e6,tb_deaths_tot*0.2/1.96,log=T)*wts[50:69]) - adj_19a  }
...
#'@param rho correlation parameter
#'@return likelihood
tb_dth_age_lLik <- function(V,rho=0.05) {
  tb_deaths_age  <- CalibDat[["tb_deaths"]][,-1]
  adj_19b        <- sum(dDirMult(M=tb_deaths_age+.1,n=tb_deaths_age,Rho=rho)*wts[50:69])
  V2 <- V[,-11]; V2[,10] <- V2[,10]+V[,11]
  sum(dDirMult(M=(V*1e6),n=tb_deaths_age,Rho=rho)*wts[50:69]) - adj_19b  }
...
#' #'@return likelihood
tot_pop19_ag_fb_lLik <- function(V,ESS=500) {
  tot_pop19_ag_fb      <- cbind(CalibDat[["tot_pop19_ag_fb"]][-9,3]/sum(CalibDat[["tot_pop19_ag_fb"]][-9,3]),
                                CalibDat[["tot_pop19_ag_fb"]][-9,4]/sum(CalibDat[["tot_pop19_ag_fb"]][-9,4]))
  adj_18               <- (sum(log(tot_pop19_ag_fb[,1])*tot_pop19_ag_fb[,1])+sum(log(tot_pop19_ag_fb[,2])*tot_pop19_ag_fb[,2]))*ESS
  V1 <- rbind(V[1,],V[2,]+V[3,],V[4,]+V[5,],V[6,],V[7,],V[8,],V[9,],V[10,]+V[11,])
  (sum(log(V1[,1]/sum(V1[,1]))*tot_pop19_ag_fb[,1])+sum(log(V1[,2]/sum(V1[,2]))*tot_pop19_ag_fb[,2]))*ESS - adj_18  }
...
tot_pop_age_lLik <- function(V,ESS=500) {
  tot_pop19_ag  <- CalibDat[["tot_pop19_ag_fb"]][-9,2]/sum(CalibDat[["tot_pop19_ag_fb"]][-9,2]);
  # data("wonder_pop",package="MITUS")
  # tot_pop16_ag<-wonder_pop/sum(wonder_pop)
  adj_18               <- sum(log(tot_pop19_ag)*tot_pop19_ag);
  sum(log(V[]/sum(V))*tot_pop19_ag[])*ESS - adj_18*ESS
}
...
US_dth_tot_lLik <- function(V) {
  US_deaths_tot   <- CalibDat[["US_tot_mort"]][c(11,21,31,41,51,61,69),-1]
  adj_20a         <- sum(dnorm(US_deaths_tot,US_deaths_tot,US_deaths_tot*0.1/1.96,log=T)*wts[c(11,21,31,41,51,61,69)])
  sum(dnorm(US_deaths_tot,V,US_deaths_tot*0.1/1.96,log=T)*wts[c(11,21,31,41,51,61,69)]) - adj_20a
}
