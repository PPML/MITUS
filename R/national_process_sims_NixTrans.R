...
#'@name  national_OutputsZint_nixtrans
...
#'@export
national_OutputsZint_nixtrans <-  function(samp_i=1,ParMatrix,loc, startyr=1950, endyr=2050,Int1=0,Int2=0,Int3=0,Int4=0,Int5=0,Scen1=0,Scen2=0,Scen3=0,prg_chng, ttt_input, care_cascade) {
  ...
  m <- temp_national_cSim( nYrs       = endyr-(startyr-1)         , nRes      = length(func3_ResNam())  , rDxt     = prms[["rDxt"]]  , TxQualt    = prms[["TxQualt"]]   , InitPop  = prms[["InitPop"]]    ,
                           ...
                           EarlyTrend = prms[["EarlyTrend"]], ag_den=prms[["aging_denom"]],  NixTrans = c(rep(1,840),rep(0,1213-840)),   trans_mat_tot_ages = trans_mat_tot_ages)$Outputs
...
#'@name  national_OutputsInt_nixtrans
#'@param loc two-digit abbreviation for location
...
#'@return out outputs
#'@export
national_OutputsInt_nixtrans <- function(loc,ParMatrix,n_cores=1,endyr=2050,Int1=0,Int2=0,Int3=0,Int4=0,Int5=0,Scen1=0,Scen2=0,Scen3=0, prg_chng, ttt_input) {
  if(min(dim(as.data.frame(ParMatrix)))==1) {
    out <- national_OutputsZint_nixtrans(samp_i=1,ParMatrix=ParMatrix,loc=loc,endyr=endyr,Int1=Int1,Int2=Int2,Int3=Int3,Int4=Int4,Int5=Int5,Scen1=Scen1,Scen2=Scen2,Scen3=Scen3, prg_chng=prg_chng, ttt_input=ttt_input)
  } else {
    out0 <- mclapply(X=1:nrow(ParMatrix),FUN=national_OutputsZint_nixtrans,mc.cores=n_cores,
