...
llikelihoodZ_rblnc <-  function(samp_i, start_mat) {
  ...
  jj <- tryCatch({
    ...
    if(sum(is.na(zz$Outputs[65,]))>0 | min(zz$Outputs[65,])<0 | min(zz$V1)<0 ) {
      ...
      v20a  <- rowSums(M[c(41,51,61),121:131])*1e6
...
#'@name llikelihood_rblnc
#'@param start_mat matrix of parameters
#'@param n_cores number of cores to use on the cluster
#'@return lLik
#'@export
llikelihood_rblnc <- function(start_mat,loc="US",n_cores=1) {
  if(loc=="US"){
    if(dim(as.data.frame(start_mat))[2]==1) {
      lLik <- llikelihoodZ_rblnc(1,t(as.data.frame(start_mat)))
    } else {
      lLik <- unlist(mclapply(1:nrow(start_mat),llikelihoodZ_rblnc,start_mat=start_mat,mc.cores=n_cores))
    }
  } else {
    if(dim(as.data.frame(start_mat))[2]==1) {
      lLik <- llikelihoodZ_rblnc_st(1,t(as.data.frame(start_mat)),loc=loc)
    } else {
      lLik <- unlist(mclapply(1:nrow(start_mat),llikelihoodZ_rblnc_st,start_mat=start_mat,loc=loc,mc.cores=n_cores))
    }}
  return((lLik)) }
