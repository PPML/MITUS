...
#' #' The CSV file is counts per 100,000; divide below for total in million
...
#'
#' #'Total Population
#' #'total population by year is given in # of millions in the CSV -- needs no further editing
#' #'example: total population is 318.558 in csv (million)
#' #'by Year
#' pop_year_fb0      <- read.csv("inst/extdata/6.15.18 Final Data/tot_by_year_fb_6-15-18.csv")
#' pop_year_fb0[,2:4]<-pop_year_fb0[,2:4]
#'
#' #'age distribution
#'
#' pop_age_fb0      <- read.csv("inst/extdata/6.15.18 Final Data/total2016_by_age_fb-6-15-18.csv")
#' pop_age_fb0[,2:4]<-pop_age_fb0[,2:4]
#'
#' CalibDat[["tot_pop_yr_fb"]]   <- pop_year_fb0
#' CalibDat[["tot_pop16_ag_fb"]] <- pop_age_fb0
#'
#' #'TB Deaths
#' #'given as absolute counts in the csv, divide by 1e6 below to get the total
...
#' zz  <- seq(-z*(1+2*(StYr-1950)/(Endyr-StYr)),z*(1+2*(2016-Endyr)/(Endyr-StYr)),by=(2*z)/(Endyr-StYr))
#' zz  <- as.numeric(EndVal)/(1+exp(-zz));    zz  }
#' ImptWeights <- LgtCurveY(1990,2016,0.90)+0.10
#' names(ImptWeights) <- 1950:2016
...
#'
#' CalibDat$US_tot_mort <- read.csv(file="~/MITUS/inst/extdata/US/US_tot_mort.csv", header = FALSE)
#' CalibDat$US_mort_age <- read.csv(file="~/MITUS/inst/extdata/US/US_mort_age.csv", header = TRUE)
#'
#' #'Save CalibDat to an .rData file
#' save(CalibDat,file=paste("~/MITUS/data/US_CalibDat_", Sys.Date(),".rda",sep=""))
