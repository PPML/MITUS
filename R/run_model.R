...
m <- cSim( nYrs     =   2100-1950,  nRes     = length(ResNam), rDxt      = rDxt     , TxQualt   = TxQualt    , InitPop   = InitPop    ,
           ...
           can_go   = can_go      , did_go=did_go           , dist_orig =dist_orig,  dist_goal=dist_goal, dist_goal_v=dist_goal_v,
           dist_orig_v=dist_orig_v )$Outputs
colnames(m) <-ResNam
#)
...
tb_graph_all(1993,2005,results)
