...
llikelihoodZ <-  function(samp_i,ParMatrix) {
  ...
  jj <- tryCatch({
    source("R/param.R")
    zz <- cSim( nYrs     =   2050-1950,  nRes     = length(ResNam), rDxt      = rDxt     , TxQualt   = TxQualt    , InitPop   = InitPop    ,
                ...
                EarlyTrend = EarlyTrend, NixTrans = NixTrans      , can_go   = can_go    , dist_goal=dist_goal    , diff_i_v  = diff_i_v   ,
                dist_orig_v=dist_orig_v)
