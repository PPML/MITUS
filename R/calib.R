calib<-function(samp_i,optim_mat, loc){
  ...
  prms <-list()
  prms <- fin_param(PV=P,loc=loc,prg_chng=prg_chng)
  ...
  zz <- fin_cSim( nYrs       = 2018-1950         , nRes      = length(func_ResNam())  , rDxt     = prms[["rDxt"]]  , TxQualt    = prms[["TxQualt"]]   , InitPop  = prms[["InitPop"]]    ,
