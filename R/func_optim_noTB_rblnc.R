...
optim_noTB_rblnc <- function(df, samp_i=1, loc="US"){
  ...
  if (loc=="US"){
    posterior = function(theta) {
      ...
      -lprior(theta) - llikelihood_noTB_rblnc(theta,loc,1)
    ...
    posterior_st = function(theta) {
      -lprior(theta) - llikelihood_noTB_rblnc(theta,loc,n_cores)
    }
