format_paramInitst<-function(){
  ...
  ########################
  ### Calc Normal values
  ParamInit <- as.data.frame(read.csv("~/MITUS/inst/extdata/ParamInitST.csv")[,2:6]); rownames(ParamInit) <- read.csv("~/MITUS/inst/extdata/ParamInitST.csv")[,1]
  ...
  #'save it all
  write.csv(ParamInit,file="~/MITUS/inst/extdata/ParamInitState_final.csv")
