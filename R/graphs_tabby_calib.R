...
tabby_calib_graphs <- function(loc) {
  ...
  if (loc=="US"){
    ...
    p <- recordPlot()
    saveRDS(p, file=paste0('~/MITUS/inst/',loc,'/calibplots/total_population.rds'))
    ...
    p <- recordPlot()
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/age_distribution.rds'))
    ...
    # p <- recordPlot()
    # saveRDS(p, paste0('~/MITUS/inst/',loc,'/age_distribution_all_ages.rds'))
    ...
    legend("bottomleft",c("Total","US born","Non-US Born","Reported data","model"),cex=0.9,
           pch=c(15,15,15,19,NA),lwd=c(NA,NA,NA,1,2),lty=c(NA,NA,NA,3,1),col=c("grey50",4,"red3",1,1),bg="white",pt.cex=c(1.8,1.8,1.8,0.3,NA))
    p <- recordPlot()
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/mortality.rds'))
    ...
    p <- recordPlot()
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/mortality_by_age.rds'))
    ...
    p <- recordPlot()
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/diagnosed_cases_2006.rds'))
    ...
    #' p <- recordPlot()
    #'
    #' saveRDS(p, paste0('~/MITUS/inst/',loc,'diagnosed_cases_2000.rds'))
    ...
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/percent_of_cases_in_non_usb.rds'))
    ...
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/percent-of-non-usb-cases-in-recent-immigrants.rds'))
    ...
    p <- recordPlot()
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/age-distribution-of-cases.rds'))
    ...
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/treatment_outcomes.rds'))
    ...
    p <- recordPlot()
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/ltbi-prev-by-age-usb.rds'))
    ...
    saveRDS(p, paste0('~/MITUS/inst/',loc,'/calibplots/ltbi-prev-by-age-non-usb.rds'))
    ...
    #        pch=c(15,15,15,19,NA),lwd=c(NA,NA,NA,1,2),lty=c(NA,NA,NA,3,1),col=c("grey50",4,"red3",1,1),bg="white",pt.cex=c(1.8,1.8,1.8,0.3,NA))
    # p <- recordPlot()
    # saveRDS(p, paste0('~/MITUS/inst/',loc,'US/mortality.rds'))
    #
    #
    # # Total Mort Age Distribution 2014 ----------------------------------------
    # V  <- cbind((df[65,255:265])+(df[65,266:276]))
    ...
    #        lwd=NA,col=c("black","gray"),bg="white")
    # p <- recordPlot()
    # saveRDS(p, paste0('~/MITUS/inst/',loc,'mortality_by_age.rds'))
