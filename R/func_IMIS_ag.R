...
#'@return lLik
#'@export
llikelihood_ag <- function(start_mat,n_cores=1) {
  if(dim(as.data.frame(start_mat))[2]==1) {
    lLik <- llikelihoodZ_ag(1,t(as.data.frame(start_mat)))
  } else {
    lLik <- unlist(mclapply(1:nrow(start_mat),llikelihoodZ_ag,start_mat=start_mat,mc.cores=n_cores))
  }
  return((lLik))
}
